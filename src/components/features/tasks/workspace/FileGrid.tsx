...
import { FileText, FileSpreadsheet, Presentation, Image, File, Download, Eye, Trash2, MoreVertical, Folder, FolderOpen } from 'lucide-react';
...
interface FileGridProps {
  ...
  files: WorkspaceFile[];
  onFolderClick?: (folder: WorkspaceFolder) => void;
  onFolderDoubleClick?: (folder: WorkspaceFolder) => void;
  ...
  if (!hasContent) {
    return (
      <div className="text-center py-12">
        ...
        <p className="text-forvis-gray-600 font-medium">This folder is empty</p>
        <p className="text-sm text-forvis-gray-500 mt-1">Upload a file or create a folder to get started</p>
      </div>
    );
  }
          ...
          <tbody className="bg-white divide-y divide-forvis-gray-200">
            ...
            {folders.map((folder) => (
              <tr 
                ...
                className="hover:bg-forvis-gray-50 transition-colors cursor-pointer"
                onClick={() => onFolderClick?.(folder)}
                onDoubleClick={() => onFolderDoubleClick?.(folder)}
              >
                <td className="px-4 py-3">
                  <div className="flex items-center gap-3">
                    <Folder className="w-5 h-5 text-forvis-blue-600" />
                    <span className="text-sm font-medium text-forvis-gray-900">{folder.name}</span>
                    {folder._count && (
                      <span className="text-xs text-forvis-gray-500">
                        ({folder._count.Files + folder._count.ChildFolders} items)
                      </span>
                    )}
                  </div>
                </td>
                <td className="px-4 py-3 text-sm text-forvis-gray-500">—</td>
                <td className="px-4 py-3 text-sm text-forvis-gray-500">—</td>
                <td className="px-4 py-3 text-sm text-forvis-gray-500">—</td>
            ...
            {/* Files after folders */}
            {files.map((file) => (
              <tr key={`file-${file.id}`} className="hover:bg-forvis-gray-50 transition-colors">
                <td className="px-4 py-3">
                  <div className="flex items-center gap-3">
        ...
        >
          ...
          <div className="p-3">
            <div className="flex items-start justify-between gap-2 mb-2">
              <h3 className="text-sm font-medium text-forvis-gray-900 truncate flex-1" title={folder.name}>
                {folder.name}
              </h3>
            </div>
            ...
            <div className="space-y-1">
              ...
              {folder.description && (
                <p className="text-xs text-forvis-gray-500 line-clamp-2">
                  {folder.description}
                </p>
              )}
            </div>
