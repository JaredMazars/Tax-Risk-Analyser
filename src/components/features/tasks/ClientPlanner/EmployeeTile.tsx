...
import { EmployeeAllocationData, GanttPosition, TimeScale, RowMetadata } from './types';
import {
  getRoleGradient,
  formatHours,
  formatPercentage,
...
// Constants
const LANE_HEIGHT = 36; // Must match ClientRow
const LANE_GAP = 2;
const MIN_DRAG_DISTANCE = 5;
const RESIZE_HANDLE_WIDTH = 8;

interface EmployeeTileProps {
  allocation: EmployeeAllocationData;
  position: GanttPosition;
  scale: TimeScale;
  columnWidth: number;
  onEdit: (allocation: EmployeeAllocationData) => void;
    ...
    >
      ...
      <div className="absolute inset-0 flex items-center px-2 text-white text-[10px] font-semibold truncate pointer-events-none z-10">
        <div className="flex items-center gap-1.5 min-w-0 flex-1">
          <span className="truncate">{getEmployeeDisplayName(allocation)}</span>
      ...
      {showTooltip && createPortal(
        ...
        >
          <div className="space-y-1">
            <div className="font-semibold text-sm">{getEmployeeDisplayName(allocation)}</div>
            {allocation.jobGradeCode && (
