...
/**
 * Partner Totals Table Component
 * 
 * Displays aggregated totals by Task Partner
 * Columns: Partner Name | Total Tasks | Net WIP | Net Revenue | Gross Profit | GP%
 */
...
interface PartnerTotalsTableProps {
...
export function PartnerTotalsTable({ tasks }: PartnerTotalsTableProps) {
  // Pagination state
  const [currentPage, setCurrentPage] = useState(1);
  const ITEMS_PER_PAGE = 20;
  
  // Sort state
  const [sortConfig, setSortConfig] = useState<{key: keyof PartnerTotal; direction: 'asc' | 'desc'} | null>(null);

  // Aggregate tasks by partner
  const partnerTotals = new Map<string, PartnerTotal>();

  tasks.forEach((task) => {
    const partnerCode = task.TaskPartner || 'Unknown';
    const partnerName = task.TaskPartnerName || task.TaskPartner || 'Unknown';
    
    if (!partnerTotals.has(partnerCode)) {
      partnerTotals.set(partnerCode, {
        partnerCode,
        partnerName,
        taskCount: 0,
        totalWIP: 0,
    ...
    const partner = partnerTotals.get(partnerCode)!;
    partner.taskCount += 1;
    partner.totalWIP += task.netWip;
    partner.ltdWipProvision += task.ltdWipProvision;
    partner.balWip += task.balWip;
    partner.grossProduction += task.grossProduction;
    partner.ltdAdj += task.ltdAdj;
    partner.netRevenue += task.netRevenue;
    partner.ltdCost += task.ltdCost;
    partner.grossProfit += task.grossProfit;
  });
  
  // Calculate percentages for each partner
  partnerTotals.forEach((partner) => {
    partner.adjustmentPercentage = partner.grossProduction !== 0 
      ? (partner.ltdAdj / partner.grossProduction) * 100 
      : 0;
    partner.grossProfitPercentage = partner.netRevenue !== 0 
      ? (partner.grossProfit / partner.netRevenue) * 100 
      : 0;
  });

  // Convert to array and sort
  let sortedPartners = Array.from(partnerTotals.values());
  ...
  if (sortConfig) {
    ...
    sortedPartners.sort((a, b) => b.netRevenue - a.netRevenue);
  }
  ...
  // Grand totals
  const grandTotals = sortedPartners.reduce(
    (acc, partner) => ({
      taskCount: acc.taskCount + partner.taskCount,
      totalWIP: acc.totalWIP + partner.totalWIP,
      balWip: acc.balWip + partner.balWip,
      netRevenue: acc.netRevenue + partner.netRevenue,
      ltdCost: acc.ltdCost + partner.ltdCost,
      grossProfit: acc.grossProfit + partner.grossProfit,
    }),
  ...
  const handleSort = (key: keyof PartnerTotal) => {
    setSortConfig((prev) => {
      if (prev?.key === key) {
  ...
  const SortIndicator = ({ columnKey }: { columnKey: keyof PartnerTotal }) => {
    ...
    <div>
      ...
      <div className="px-4 py-3 bg-forvis-gray-50 border-b border-forvis-gray-200">
        <p className="text-sm text-forvis-gray-600">
          <span className="font-semibold text-forvis-gray-900">{sortedPartners.length}</span> partners •{' '}
            ...
            {paginatedPartners.map((partner, index) => (
              ...
              >
                <td className="px-4 py-3">
                  <div>
                    <span className="font-medium text-forvis-gray-900">{partner.partnerName}</span>
                    <span className="ml-2 text-xs text-forvis-gray-500">{partner.partnerCode}</span>
                  </div>
                </td>
                <td className="px-4 py-3 text-right tabular-nums">{formatNumber(partner.taskCount)}</td>
                <td className={`px-4 py-3 text-right tabular-nums ${partner.balWip < 0 ? 'text-forvis-error-600' : ''}`}>
                  {formatCurrency(partner.balWip)}
                </td>
                <td className={`px-4 py-3 text-right tabular-nums ${partner.totalWIP < 0 ? 'text-forvis-error-600' : ''}`}>
                  {formatCurrency(partner.totalWIP)}
                </td>
                <td className={`px-4 py-3 text-right tabular-nums ${partner.netRevenue < 0 ? 'text-forvis-error-600' : ''}`}>
                  {formatCurrency(partner.netRevenue)}
                </td>
                <td className="px-4 py-3 text-right tabular-nums">{formatCurrency(partner.ltdCost)}</td>
                <td className={`px-4 py-3 text-right tabular-nums ${partner.grossProfit < 0 ? 'text-forvis-error-600' : ''}`}>
                  {formatCurrency(partner.grossProfit)}
                </td>
                <td className={`px-4 py-3 text-right tabular-nums ${partner.grossProfitPercentage < 0 ? 'text-forvis-error-600' : ''}`}>
                  {formatPercentage(partner.grossProfitPercentage)}
                </td>
              </tr>
            ))}
          </tbody>
