...
interface WIPAgingGroupTotalsTableProps {
...
export function WIPAgingGroupTotalsTable({ tasks }: WIPAgingGroupTotalsTableProps) {
  ...
  const groupedData = useMemo(() => {
    const groupMap = new Map<string, GroupAggregation>();

    tasks.forEach((task) => {
      const key = task.groupCode || 'UNKNOWN';
        ...
        <div className="bg-white">
          {paginatedData.map((group, index) => (
            ...
            >
              <div className="text-forvis-gray-900 truncate" title={`${group.groupCode} - ${group.groupDesc || 'No description'}`}>
                ...
                {' - '}
                {group.groupDesc || 'No description'}
              </div>
              <div className="text-right tabular-nums text-forvis-gray-600">
                {group.taskCount}
              </div>
              <div className={`text-right tabular-nums ${group.curr !== 0 ? 'text-forvis-gray-900' : 'text-forvis-gray-400'}`}>
                {formatCurrency(group.curr)}
              </div>
              <div className={`text-right tabular-nums ${group.bal30 !== 0 ? 'text-forvis-gray-900' : 'text-forvis-gray-400'}`}>
                {formatCurrency(group.bal30)}
              </div>
              <div className={`text-right tabular-nums ${group.bal60 !== 0 ? 'text-forvis-warning-600' : 'text-forvis-gray-400'}`}>
                {formatCurrency(group.bal60)}
              </div>
              <div className={`text-right tabular-nums ${group.bal90 !== 0 ? 'text-forvis-warning-600' : 'text-forvis-gray-400'}`}>
                {formatCurrency(group.bal90)}
              </div>
              <div className={`text-right tabular-nums ${group.bal120 !== 0 ? 'text-forvis-error-600' : 'text-forvis-gray-400'}`}>
                {formatCurrency(group.bal120)}
              </div>
              <div className={`text-right tabular-nums ${group.bal150 !== 0 ? 'text-forvis-error-600' : 'text-forvis-gray-400'}`}>
                {formatCurrency(group.bal150)}
              </div>
              <div className={`text-right tabular-nums font-semibold ${group.bal180 !== 0 ? 'text-forvis-error-700' : 'text-forvis-gray-400'}`}>
                {formatCurrency(group.bal180)}
              </div>
              <div className={`text-right tabular-nums font-semibold ${
                group.balWip < 0 ? 'text-forvis-error-600' : 'text-forvis-blue-600'
              }`}>
              ...
              <div className={`text-right tabular-nums font-bold ${
                group.nettWip < 0 ? 'text-forvis-error-600' : 'text-forvis-blue-700'
              }`}>
                {formatCurrency(group.nettWip)}
              </div>
            </div>
          ))}
          ...
          >
            <div className="text-forvis-blue-800">
              GRAND TOTAL ({grandTotals.groupCount} groups)
            </div>
