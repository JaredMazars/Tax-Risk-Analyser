...
/**
 * WIP Aging Master Service Line Totals Table Component
 * 
 * Aggregates WIP aging data by master service line.
 */
...
interface MasterServiceLineAggregation {
...
export function WIPAgingMasterServiceLineTotalsTable({ tasks }: WIPAgingMasterServiceLineTotalsTableProps) {
  const [currentPage, setCurrentPage] = useState(1);
  const ITEMS_PER_PAGE = 25;
  
  // Sort state
  const [sortConfig, setSortConfig] = useState<{key: keyof MasterServiceLineAggregation; direction: 'asc' | 'desc'} | null>(null);
  
  // Handle sort
  const handleSort = (key: keyof MasterServiceLineAggregation) => {
  ...
  const groupedData = useMemo(() => {
    const slMap = new Map<string, MasterServiceLineAggregation>();

    tasks.forEach((task) => {
      const key = task.masterServiceLineCode || 'UNKNOWN';
      ...
      if (existing) {
        ...
        slMap.set(key, {
          masterServiceLineCode: key,
          masterServiceLineName: task.masterServiceLineName || key,
          taskCount: 1,
    ...
    return aggregated.sort((a, b) => a.masterServiceLineName.localeCompare(b.masterServiceLineName));
  }, [tasks, sortConfig]);
    ...
    return (
      <div className="text-center py-12">
        ...
        <p className="mt-1 text-sm text-forvis-gray-500">
          No master service lines match the current filters.
        </p>
      </div>
    );
  }
      ...
      <div className="inline-block min-w-full align-middle">
        ...
        <div className="bg-white">
          {paginatedData.map((item, index) => (
            <div
              key={item.masterServiceLineCode}
            ...
            >
              <div className="text-forvis-gray-900 truncate font-medium" title={item.masterServiceLineName}>
