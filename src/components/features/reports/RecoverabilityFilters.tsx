...
/**
 * Recoverability Report Filters Component
 * 
 * Enhanced filtering with SearchMultiCombobox for debtor data:
...
interface RecoverabilityFiltersProps {
  filters: RecoverabilityFiltersState;
  onFiltersChange: (filters: RecoverabilityFiltersState) => void;
  availableClients: ClientDebtorData[];
}

export function RecoverabilityFilters({
  filters,
  onFiltersChange,
  availableClients,
}: RecoverabilityFiltersProps) {
  ...
  // Extract unique options from available clients
  const { clientOptions, serviceLineOptions, groupOptions, masterServiceLineOptions, subServiceLineGroupOptions } = useMemo(() => {
    ...
    availableClients.forEach((client) => {
      if (!clientMap.has(client.GSClientID)) {
        clientMap.set(client.GSClientID, {
          id: client.GSClientID,
          label: `${client.clientCode} - ${client.clientNameFull || 'Unnamed Client'}`,
        });
      }
    });
    ...
    availableClients.forEach((client) => {
      if (!serviceLineMap.has(client.servLineCode)) {
        serviceLineMap.set(client.servLineCode, {
          id: client.servLineCode,
          label: client.serviceLineName,
        });
      }
    });
    ...
    availableClients.forEach((client) => {
      if (!groupMap.has(client.groupCode)) {
        groupMap.set(client.groupCode, {
          id: client.groupCode,
          label: client.groupDesc,
        });
      }
    });
    ...
    availableClients.forEach((client) => {
      if (!masterServiceLineMap.has(client.masterServiceLineCode)) {
        masterServiceLineMap.set(client.masterServiceLineCode, {
          id: client.masterServiceLineCode,
          label: client.masterServiceLineName,
        });
      }
    });
    ...
    availableClients.forEach((client) => {
      const code = client.subServlineGroupCode || client.servLineCode;
      if (!subServiceLineGroupMap.has(code)) {
        subServiceLineGroupMap.set(code, {
          id: code,
          label: client.subServlineGroupDesc || client.serviceLineName,
        });
      }
    });
