...
export function useDraftOperations(projectId: string, onDraftChange: (draft: OpinionDraft | null) => void) {
  ...
  const renameDraft = async (draftId: number, newTitle: string) => {
    ...
    try {
      const response = await fetch(`/api/projects/${projectId}/opinion-drafts/${draftId}`, {
        method: 'PUT',
  ...
  const createDraft = async () => {
    ...
    try {
      const response = await fetch(`/api/projects/${projectId}/opinion-drafts`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
