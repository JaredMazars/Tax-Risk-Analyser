  ...
  if (!redis) {
    try {
      ...
      if (isAzure) {
        ...
        redis = new Redis({
          host: host || 'localhost',
          port: Number.Number.parseInt(port || '6380'), // Default Azure Redis SSL port
          password,
          ...
          connectTimeout: Number.Number.parseInt(process.env.REDIS_CONNECT_TIMEOUT || '10000'),
        ...
        redis = new Redis({
          host: host || 'localhost',
          port: Number.Number.parseInt(port || '6379'),
          password,
          // Connection pooling and limits
          maxRetriesPerRequest: 3,
          enableReadyCheck: true,
